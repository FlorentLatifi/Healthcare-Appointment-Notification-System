...
describe('BookAppointmentPage', () => {
  ...
  async function fillAndSubmit(datetime, reason) {
    if (datetime) {
      ...
      fireEvent.change(dtInput, { target: { value: datetime } });
    }
    if (reason) {
      const ta = document.querySelector('textarea');
      await userEvent.clear(ta);
      await userEvent.type(ta, reason);
    }
  ...
  it('rejects non-30-minute intervals', async () => {
    ...
    expect(await screen.findByText(/time must be in 30-minute intervals/i)).toBeInTheDocument();
    expect(mockApiClient.post).not.toHaveBeenCalled();
  });
